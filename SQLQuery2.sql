...
--GROUP BY is used to group rows with the same values so you can apply aggregation (like COUNT(), SUM(), etc.).
--ORDER BY is used to sort the final result, either in ascending or descending order.
...
--Not understanding when to use DISTINCT

--4.	 When would you use COUNT (DISTINCT ...), AVG(...), and SUM(...) together? 
--Use them together to generate summarized reports, like:
--Unique counts
--Averages of ratings or scores
--Total revenue, cost, etc.

--5.	How does GROUP BY affect query performance, and how can indexes help?
--GROUP BY can be slow if the table has a lot of data.
--Adding an index on the column you're grouping can make the query faster.
